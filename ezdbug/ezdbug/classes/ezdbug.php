<?php
function ezDbugDump($var, $type = false) {
	if (!EZDBUG) 
	{
		return false;
	}
	ezDbugSetDebugHTML();
	if (is_string($type) && ($type == 'array' || $type == 'object' || $type == 'xml') ) 
	{
		$d = new dbug($var, $type);
		} else 
	{
...
function ezDbugLogDump($var, $type = false, $filename = 'debug.html') {
	if (!EZDBUG)
	{
		return false;
	}
	...
	if (is_string($type) && ($type == 'array' || $type == 'object' || $type == 'xml') ) 
	{
		$ld = new dbug($var, $type);
		} else 
	{
			$ld = new dbug($var);
	}
	$log_string = ob_get_flush();
	
	ezDbugLogHTML($log_string, $filename);
	return true;
}

function ezDbugLogHTML($msg, $filename = 'debug.html'){
	if (!EZDBUG) 
	{
		return false;
	}
	$msg = $msg . '<br><br>';
	$html_path = EZDBUG_LOG_DIR . '/' . $filename;
	if (!file_exists($html_path) ) 
	{
		if (!touch($html_path) )
		{
			exit("Couldn't write $html_path");
		} else {
			$h = fopen($html_path, 'a');
			fwrite($h, self::GetDebugHTML());
		}
	} else
	{
		$h = fopen($html_path, 'a');
	...
	{ 
		fclose($h);
		exit("Couldn't open " . EZDBUG_LOG_DIR . $filename . " for logging.");
	}
	fwrite($h, $msg);
	fclose($h);
	return true;
...
function ezDbugLogString($str, $filename = 'log.txt') {
	if (!EZDBUG) 
	{
		return false;
	}
	$str = $str . "\n\n";
	if (!is_dir(EZDBUG_LOG_DIR)) {
		die('Error logging failed: ' . EZDBUG_LOG_DIR . ' is not a directory.');
	}
	
	$log_path = EZDBUG_LOG_DIR . '/'. $filename;
	
	if (!file_exists($log_path)) {
		if (!touch($log_path)) {
			die('Error logging failed: Could not access ' . $log_path);
		}
	}
	...
	if (!$h)
	{ 
		die("Couldn't open $log_path for logging.");
	}
	fwrite($h, $str);
	fclose($h);
	return true;
...
function ezDbugSetDebugHTML() {
	if (!isset($GLOBALS['__DebugHTMLIsSet']))
	{
		echo ezDbugGetDebugHTML();
	}
	$GLOBALS['__DebugHTMLIsSet'] = true;
}
/**
 * Do not call this directly. This is a workaround for php 4's lack of 
 * static class variables.
 */	 	
function ezDbugGetDebugHTML() {
...
class dBug {
	...
	function varIsArray($var) {
		...
		if(is_array($var)) {
			foreach($var as $key=>$value) {
				...
				if(in_array(gettype($value),$this->arrType) && $this->CheckDepth() )
	...
	function CheckDepth() {
		if ($GLOBALS['__ezDbugDepth'] >= $GLOBALS['__ezDbugMaxDepth'])
		{
			return false;
		}
		return true;
	...
	function varIsObject($var) {
		...
		if(is_object($var)) {
			...
			if ($GLOBALS["__ezDbugShowFunctions"]) {
